...
import GameManagerS2 from "../GameManager/GameManagerS2";

const { ccclass, property } = cc._decorator;
declare const firebase: any;

@ccclass
export default class PlayerGhost extends cc.Component {
    ...
    updateOthersCurrentPos() { // 定期自動修正其他玩家正確位置
        ...
        this.scheduleOnce(() => {
            ...
            if(Math.abs(this.node.position.x-posX)>320 || Math.abs(this.node.position.y-posY)>320){ // 如果位置差太多就直接閃現。(320為10格)
